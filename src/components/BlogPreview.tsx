...
const BlogPreview = ({ post, isFeatured = false }: BlogPreviewProps) => {
  ...
  const excerpt = post.excerpt || getTextExcerpt(post.content, isFeatured ? 160 : 100);
    ...
    <article className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden group">
      <Link to={`/post/${post.slug}`} className="block">
        ...
        <div className="p-3">
          ...
          <h3 className="text-sm font-semibold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors duration-200 leading-tight line-clamp-2">
            {post.title}
          </h3>
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex items-center flex-wrap gap-1">
              ...
              {post.tags.slice(0, 2).map((tag, index) => (
                <span
                  key={index}
                  className="px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded-full text-xs font-medium hover:bg-gray-200 transition-colors"
                >
                  {tag}
                </span>
              ))}
